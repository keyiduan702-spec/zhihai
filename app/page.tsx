...
export default function Home() {
  ...
  return (
    <main className="app-shell">
      ...
      {view === "notes" && readerOpen && activeNote && <div className="note-reader-backdrop"><article className="card markdown note-editor note-reader" style={{ "--reader-scale": readerZoom / 100 } as CSSProperties}><div className="markdown-top"><div><strong>{activeNote.title}.md</strong><span>{noteStatus || "内容自动保存在本地 D1 数据库"}</span></div><div className="reader-actions"><button aria-label="缩小笔记" disabled={readerZoom <= 80} onClick={() => setReaderZoom((zoom) => Math.max(80, zoom - 10))}>−</button><button className="zoom-value" onClick={() => setReaderZoom(100)}>{readerZoom}%</button><button aria-label="放大笔记" disabled={readerZoom >= 160} onClick={() => setReaderZoom((zoom) => Math.min(160, zoom + 10))}>＋</button>{editingNote ? <><button onClick={() => setEditingNote(false)}>取消</button><button onClick={saveNote}>保存</button></> : <button onClick={() => setEditingNote(true)}>编辑</button>}<button aria-label="关闭笔记" onClick={() => setReaderOpen(false)}>×</button></div></div><span className="crumb">学习笔记 / 本地知识库</span>{editingNote ? <><label>标题<input className="note-title-editor" value={draftTitle} onChange={(event) => setDraftTitle(event.target.value)} /></label><label>正文<textarea className="note-content-editor" value={draftContent} onChange={(event) => setDraftContent(event.target.value)} placeholder="输入 Markdown 或普通文字…" /></label></> : <div className="markdown-body"><h1>{activeNote.title}</h1><ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeHighlight]}>{activeNote.content || "_这篇笔记暂无正文。_"}</ReactMarkdown></div>}</article></div>}
    </main>
  );
}
